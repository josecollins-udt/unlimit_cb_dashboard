...
def _aggregate_dashboard_metrics(df: pd.DataFrame) -> dict:
    ...
    won_df = df[won_mask]
    if not won_df.empty and pd.notna(won_df["result_date"].max()):
        last_won_date = pd.to_datetime(won_df["result_date"].dt.date.max())
        now_date = pd.to_datetime(datetime.now().date())
        last_won_days = (now_date - last_won_date).days
        
        last_won_cases = won_df[pd.to_datetime(won_df["result_date"].dt.date) == last_won_date]
        last_won_count = int(len(last_won_cases))
        last_won_amount = float(last_won_cases["amount"].sum())
        last_won_date_str = last_won_date.strftime('%Y-%m-%d')
        
        if last_won_days < 20:
            lw_bg, lw_border, lw_text, lw_subtext = "#E8F5E9", "#4CAF50", "#2E7D32", "#388E3C"
        elif last_won_days <= 40:
            lw_bg, lw_border, lw_text, lw_subtext = "#FFF3E0", "#FF9800", "#E65100", "#EF6C00"
        else:
            lw_bg, lw_border, lw_text, lw_subtext = "#FFEBEE", "#F44336", "#C62828", "#D32F2F"
    else:
        last_won_days = "-"
        last_won_count = 0
        last_won_amount = 0.0
        last_won_date_str = "-"
        lw_bg, lw_border, lw_text, lw_subtext = "#F5F5F5", "#9E9E9E", "#424242", "#616161"
    ...
    return {
        ...
        "last_won_count": last_won_count,
        "last_won_amount": last_won_amount,
        "last_won_date_str": last_won_date_str,
        "lw_bg": lw_bg,
        "lw_border": lw_border,
        "lw_text": lw_text,
        "lw_subtext": lw_subtext,
...
def process_data(df: pd.DataFrame) -> dict:
    ...
    if not df.empty:
        ...
        for idx, row in df_display.iterrows():
            ...
            if status_val in STATUS_CONFIG:
                stss.add(STATUS_CONFIG[status_val]['label'])
            elif pd.notnull(status_val):
                stss.add(status_val)
            else:
                stss.add('N/A')
  ...
  <div class="header">
    <div class="title-group">
      <div style="display: flex; align-items: center; gap: 16px; flex-wrap: wrap;">
        ...
        <div style="background: {data['all']['lw_bg']}; border: 1px solid {data['all']['lw_border']}; border-radius: 8px; padding: 6px 12px; display: flex; flex-direction: column; align-items: center; justify-content: center;">
          <span style="color: {data['all']['lw_text']}; font-weight: 700; font-size: 0.85rem;">Días sin casos ganados: {data['all']['last_won_days']}</span>
          <span style="color: {data['all']['lw_subtext']}; font-size: 0.75rem; margin-top: 2px;">Último ({data['all']['last_won_date_str']}): {data['all']['last_won_count']} casos / ${data['all']['last_won_amount']:,.2f}</span>
        </div>
      </div>
...
def main():
    ...
    out_path = os.path.join(os.path.dirname(__file__), "dashboard_output_jenkins_local.html")
